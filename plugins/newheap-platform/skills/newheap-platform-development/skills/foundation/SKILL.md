...
Report the selected profile, enabled capabilities, package versions, checks actually run and anything still unverified. Once the foundation is ready, return to the `newheap-platform-development` router and load only the focused internal module needed for each requested feature.
