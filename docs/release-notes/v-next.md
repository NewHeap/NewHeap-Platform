...
| The number of tool selectors per agent is configurable through `NhAssistantLimits.MaxToolSelectorsPerAgent` (default 128) for code and administrator agents; code agents are checked at host start instead of in `AddAgent`, and the stored selector list stays bounded by `MaxStoredToolSelectorCharacters` (8,000). | Optional: raise it with `WithLimits`; also raise `MaxToolsPerAgent` when a turn must offer more than 64 tools. |
...
| The thread collapses consecutive tool calls into one `nh-assistant-tool-call-group` line that names the running tool and counts finished and failed calls; expanding it shows the individual cards. Approvals and text break a group. The working indicator now also appears between tool calls and shows the elapsed seconds. | No action; hosts that override `nh-assistant.*` texts can add the `tool-group` and `thread.elapsed` keys. |
