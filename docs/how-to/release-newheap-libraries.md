...
3. Bootstrap the first version of each npm package if npm requires an existing package before trusted publishing can be configured. Create a short-lived granular npm token that can publish new public packages in the `@newheap` scope, enables bypass 2FA only for the bootstrap, and expires as soon as practical. Store it temporarily as the `NPM_BOOTSTRAP_TOKEN` repository secret. Run **Bootstrap npm package** from `main` once for each npm unit; the workflow refuses existing package names, packages through the normal artifact validator, publishes with public access, and verifies the exact version anonymously. Immediately configure each package's trusted publisher for the public repository and the calling workflow `release-contract.yml`, delete the repository secret, revoke the npm token, and remove the temporary bootstrap workflow in the next cleanup commit.
