...
namespace WebAPI.Controllers
{
    ...
    public class AccountController : BaseController<AccountController, User>
    {
        ...
        private bool IsLockedOut(User user)
        {
            ...
            var inLockOutPeriod = user.LockoutStart <= DateTimeOffset.Now && (lockoutEnd == null || lockoutEnd >= DateTimeOffset.Now);
        ...
        public async Task<IActionResult> Authorize([FromBody] LoginAccountMutateModel model)
        {
            ...
            if (!ModelState.IsValid || user == null)
            {
                await _dbLogService.LogAsync(
                    message: "Failed login attempt for user {0}.",
            ...
            if (result.Succeeded)
            {
                ...
                await _dbLogService.LogAsync(
                    message: "User login succesfull {0}.",
                    messageArguments: new string[] {
                                user.Id.ToString()
                    },
                    objectId: user.Id.ToString(),
            ...
            await _dbLogService.LogAsync(
                message: "Invalid application state when authorizing.",
                objectId: user.Id.ToString(),
                objectType: (typeof(User)).Name,
                objectTypeFull: (typeof(User)).FullName,
                userId: UserId,
        ...
        public async Task<IActionResult> GetAccount()
        {
            ...
            var divisionsQuery = _userRepository
                .GetDbSet<Division>() as IQueryable<Division>;

            if (!User.HasClaim(x => x.Type == NhPlatformClaimTypes.Permission && x.Value == NewHeap.Platform.Common.Constants.DivisionPermissionClaimValues.AccessAll))
            {
            ...
            if (user.ActiveDivisionId.HasValue && !await _userManager.DivisionAccessAsync(user.ActiveDivisionId, 
                claims, new List<Claim>() { new Claim(NhPlatformClaimTypes.DivisionPermission, NewHeap.Platform.Common.Constants.DivisionPermissionClaimValues.GeneralView) }))
            {
                user.ActiveDivisionId = null;
                await _userRepository.SaveChangesAsync();
            }

            var removeDivisions = new List<Division>();
            foreach (var division in divisions)
            {
                if (!await _userManager.DivisionAccessAsync(division.Id, claims, new List<Claim>() { new Claim(NhPlatformClaimTypes.DivisionPermission, NewHeap.Platform.Common.Constants.DivisionPermissionClaimValues.GeneralView) }))
                {
