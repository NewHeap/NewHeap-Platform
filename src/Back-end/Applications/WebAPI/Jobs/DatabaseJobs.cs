...
namespace WebAPI.Jobs
{
    public class DatabaseJobs
    {
        ...
        private async Task SeedRoles(IServiceScope scope)
        {
            ...
            Func<UserRole, IEnumerable<Claim>, System.Threading.Tasks.Task> addIdentityRoleClaims = async (UserRole identityRole, IEnumerable<Claim> claims) =>
            {
                ...
                var removed = identityRoleClaims.Where(x => !claims.Any(y => x.Type == y.Type && x.Value == y.Value))
                    .ToList();
            ...
            {
                var identityRole = new UserRole(role);
                ...
                {
                    var identityRoleClaims = new List<Claim> {
                ...
                var defaultPermissionRoles = new string[]
                {
                    "User",
                };
        ...
        private async Task SeedUsers(IServiceScope scope, AppDbContext dbContext)
        {
            ...
            if (user == null)
            {
                ...
                //Add roles
                await userManager.AddToRoleAsync(await userManager.FindByNameAsync("info@newheap.com"), "SuperAdministrator");
            }
            ...
            if (backgroundWorkerUser == null)
            {
                ...
                var result = await userManager.CreateAsync(userToInsert, configuration["AppSettings:SystemUserPassword"]);

                //Add roles
                await userManager.AddToRoleAsync(await userManager.FindByNameAsync(backgroundWorkerUserEmail), "SuperAdministrator");
            }
        ...
        {
            string[] divisionRoleNames = new string[] { 
                "SuperAdministrator",
                "Administrator",
                "User"
            };
            ...
            Func<DivisionRole, IEnumerable<Claim>, Task> addDivisionRoleClaims = async (DivisionRole divisionRole, IEnumerable<Claim> claims) =>
            {
                var divisionRoleClaimRepository = divisionManager.GetRoleClaimRepository();
                var divisionRoleClaims = await divisionRoleClaimRepository.GetAll().Where(x => x.DivisionRoleId == divisionRole.Id).ToListAsync();

                var newRoleClaims = new List<DivisionRoleClaim>();

                foreach (var claim in claims)
                {
                    var existingClaim = divisionRoleClaims.FirstOrDefault(x => x.ClaimType.Equals(claim.Type) && x.ClaimValue.Equals(claim.Value));
                    if (null == existingClaim)
                    {
                        newRoleClaims.Add(new DivisionRoleClaim() { 
                            DivisionRoleId = divisionRole.Id,
            ...
            await divisionManager.GetRoleRepository().GetAll().Where(x => !divisionRoleNames.Contains(x.Name)).ExecuteDeleteAsync();
            ...
            foreach (var divisionRole in divisionRoles)
            {
                ...
                {
                    var divisionRoleClaims = new Claim[] {
