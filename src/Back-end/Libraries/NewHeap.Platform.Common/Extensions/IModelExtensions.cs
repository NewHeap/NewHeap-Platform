...
public static class IModelExtensions
{
    public static string Column<T>(this IModel model, Expression<Func<T, object?>> prop, bool prefixTable = true)
    {
        ...
        if (memberName == null)
        {
            throw new InvalidOperationException(
                $"Property {memberExpression.Member.Name} not found in entity type {typeof(T).Name}");
        }
        ...
        var tableName = string.Join('.',
            entityDefinition.GetSchemaQualifiedTableName()!.Split('.').Select(x => $"[{x}]"));
    ...
    public static string Table<T>(this IModel model)
    {
        ...
        var table = entityDefinition.GetSchemaQualifiedTableName();
        if (table == null)
        {
