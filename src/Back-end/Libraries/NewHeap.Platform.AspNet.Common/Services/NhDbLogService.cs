    ...
    where TDivisionRoleClaim : NhDivisionRoleClaim
{
    ...
    {
        var targetDbContext = dbContext ?? _logRepository.Context;
        ...
        await targetDbContext.Set<TLog>().AddAsync(log, cancellationToken);
        ...
        if (doSaveChanges)
        {
            await SaveChanges(targetDbContext, cancellationToken);
        }
    ...
    {
        await SaveChanges(_logRepository.Context);
    }

    public virtual async Task SaveChanges(DbContext targetDbContext, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(targetDbContext);
