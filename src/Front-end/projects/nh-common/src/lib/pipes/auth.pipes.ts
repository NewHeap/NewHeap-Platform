...
@Pipe({
    name: "isAllPermissionsGranted",
    standalone: false
})
export class IsAllPermissionsGrantedPipe implements PipeTransform {

  constructor(private authService: NhAuthService) {
  }

  transform(permissions: Array<string>): boolean {
    return this.authService.isAllPermissionsGranted(permissions);
  }
}
