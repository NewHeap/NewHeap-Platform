...
| NewHeap.Platform.AI.Common, ASP.NET, Generators, MCP, Agent Framework and Test | named model profiles, keyed Microsoft clients, authorized actor/division context, stable tool descriptors and schema manifests, authorized discovery, generated `AIFunction` catalogs, official MCP and Agent Framework adaptation, scoped capabilities, canonical proposals and approvals, budget/concurrency/idempotency guards, independent verification, authorization-first hierarchical context with provenance and trust, versioned instruction manifests, fail-closed invocation gates and deterministic test clients | SPM-219–227 | consumer-owned project-assistant resolution, server-authorized division/capability context, default-deny discovery, local and official in-memory MCP project search, one approved idempotent and independently verified project mutation, a policy-bound non-human project agent, active-division project context with prompt-injection evidence, a content-free project-agent instruction manifest, and focused adversarial tests |
