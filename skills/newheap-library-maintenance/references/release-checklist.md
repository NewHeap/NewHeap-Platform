...
Before any package push, confirm the source repository is public, every package name is owned by NewHeap, and npm/nuget.org trusted-publisher policies match the calling workflow. After the push, require anonymous exact-version checks for every target. For an individual NuGet or npm unit, confirm the successful top-level run queues the guarded plugin patch follow-up; plugin and `all` runs must not recurse. Never add a long-lived publication token as the normal release path.
