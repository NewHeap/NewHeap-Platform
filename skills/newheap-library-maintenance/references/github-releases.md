...
- Stable: leave every release version unchanged in feature branches. Run `Prepare release` from `main` and select one unit or `all`, plus the SemVer bump; this selection is the only normal manual action. The workflow validates the existing public API snapshot, updates versions, regenerates guidance and its single immutable-evidence catalog, and refreshes the snapshot for the expected guidance-version change. It then creates one generated `release/*` commit, invokes the reusable release contract, fast-forwards unchanged `main` to the validated commit, and publishes that exact SHA. Registry publication and GitHub Release finalization are separate jobs; finalization retries delayed public-registry indexing and requires every exact package version to be anonymously readable before publishing complete drafts. After a successful individual NuGet or npm release, the top-level run queues a separate plugin patch release so the published plugin captures the regenerated package compatibility metadata. A plugin release cannot queue itself, and `all` already includes it. If `main` advances before the update, start Prepare release again instead of rebasing the generated branch.
...
The public repository, registry owners and trusted-publisher identities are already established; do not repeat bootstrap work for normal releases. When `release/manifest.json` gains a genuinely new package name, verify ownership or availability and follow the exceptional onboarding procedure in `docs/how-to/release-newheap-libraries.md`. A new package name does not itself force a major bump: patch or minor remains valid while the existing package contracts stay compatible. Never attach new package membership to a unit version that is already public, and never retain a temporary bootstrap token or workflow.
